...
class LinuxRPMFile:
    ...
    def GenerateSpecFile(self):
        with open(f"{self.intermediateDir}/rpm.spec", 'w') as specfile:
            ...
            elif self.variables["PFDISTRO"] in ["SUSE", "ULINUX"]:
            ...
            for d in self.sections["Directories"]:
                if d.type != "sysdir":
                    specfile.write(f'%defattr({str(d.permissions)},{d.owner},{d.group}' + ")\n")
    ...
    def BuildPackage(self):
        ...
        retval = os.system(
            f"rpmbuild --buildroot {self.stagingDir} -bb {self.specfileName}"
        )
        if retval != 0:
